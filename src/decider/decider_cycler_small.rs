...
#[derive(Debug)]
pub struct DeciderCyclerSmall {
    // TODO make DeciderData64
...
impl DeciderCyclerSmall {
    ...
    // Machine is copied to assure L1 cache usage
    fn decide_machine_binary(&mut self, machine: MachineBinary) -> MachineStatus {
        #[cfg(feature = "enable_html_reports")]
        self.data.write_html_file_start(
            Self::decider_id(),
            &MachineId::new_option_id(self.machine_id, machine),
        );
...
impl Decider for DeciderCyclerSmall {
...
mod tests {
    ...
    fn is_cycler(machine: &MachineId) -> bool {
        let config = Config::builder(machine.n_states())
            ...
            .build();
        let check_result = DeciderCyclerSmall::decide_single_machine(&machine, &config);
    ...
    fn decider_cycler_holds_after_107_steps() {
        ...
        let config = Config::builder(machine.n_states())
            ...
            .step_limit_decider_cycler(5000)
            .build();
        let res = DeciderCyclerSmall::decide_single_machine(&machine, &config);
    ...
    fn decider_cycler_unspecified() {
        ...
        let config = Config::builder(machine.n_states())
            .write_html_file(true)
            .build();
        let machine_status = DeciderCyclerSmall::decide_single_machine(&machine, &config);
