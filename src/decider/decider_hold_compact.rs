...
//! This is just a test for a compact long tape and later speed up. \
...
use crate::{
    ...
    decider::{
        self,
        decider_data_compact::DeciderDataCompact,
...
pub struct DeciderHoldCompact {
    data: DeciderDataCompact,
    // machine id, just for debugging
    // machine_id: IdBig,
}

impl DeciderHoldCompact {
    pub fn new(config: &Config) -> Self {
        #[allow(unused_mut)]
        let mut decider = Self {
            data: DeciderDataCompact::new(config),
            // machine_id: 0,
        };
...
impl Decider for DeciderHoldCompact {
...
impl Display for DeciderHoldCompact {
...
pub fn test_decider_hold_u128(tm_text_format: &str) {
    ...
    let config = Config::builder(machine.n_states())
        ...
        .build();
    let check_result = DeciderHoldCompact::decide_single_machine(&machine, &config);
    println!("{}", check_result);
...
mod tests {
    ...
    fn decider_hold_u128_applies_bb4_max() {
        ...
        let mut decider = DeciderHoldCompact::new(&config);
